...
export const useOnboardingForm = () => {
  ...
  const [experience, setExperience] = useState<"beginner" | "intermediate">("intermediate");
  const [goalFocus, setGoalFocus] = useState<string>("general_fitness");
  const [preferredMuscles, setPreferredMuscles] = useState<string>("");
  const [constraints, setConstraints] = useState<string>("");
  const [sessionLength, setSessionLength] = useState<string>("30-45");
  ...
  const tPathDescriptions = {
    ulul: {
      ...
      pros: ["Higher frequency per muscle group (2x/week)", "Good for hypertrophy", "Flexible scheduling"],
      cons: ["Sessions can be longer", "Potential for upper body fatigue", "Less focus on single 'big lift' days"],
      research: ["Studies show that training muscle groups twice a week can lead to greater muscle growth than once a week.", "Allows for more recovery time for individual muscle groups compared to full-body splits.", "Provides a balanced approach to training, hitting all major muscle groups effectively."]
    },
    ppl: {
      title: "3-Day Push/Pull/Legs (PPL)",
      pros: ["Logical split by movement pattern", "Allows for high volume per session", "Feels intuitive"],
      cons: ["Lower frequency per muscle group (once every 5-7 days)", "Missing a day can unbalance the week", "Can be demanding for beginners"],
      research: ["PPL is effective for building strength and muscle mass by allowing high volume for specific movement patterns.", "Grouping exercises by push, pull, and legs can optimize recovery and performance for each session.", "This split is popular among intermediate to advanced lifters for its structured approach to progressive overload."]
    }
  };
  ...
  const handleNext = useCallback(() => {
    if (currentStep === 4 && equipmentMethod === 'skip') {
      setCurrentStep(6); // Skip from Schedule & Tools to App Features
      return;
    }
    if (currentStep < 7) { // There are 6 main steps before the summary
  ...
  const handleBack = useCallback(() => {
    if (currentStep === 6 && equipmentMethod === 'skip') {
      setCurrentStep(4); // Go back from App Features to Schedule & Tools
      return;
    }
    if (currentStep > 1) {
      setCurrentStep(prev => prev - 1);
  ...
  const handleSubmit = useCallback(async (fullName: string, heightCm: number, weightKg: number, bodyFatPct: number | null) => {
    ...
    try {
      const payload = {
        tPathType, experience, goalFocus, preferredMuscles, constraints, sessionLength,
        equipmentMethod, gymName,
  ...
  return {
    ...
    handleNext, handleBack, handleSubmit, identifiedExercises, addIdentifiedExercise,
