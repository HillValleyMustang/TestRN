...
interface WorkoutSelectorProps {
  ...
  // Props from useWorkoutFlowManager
...
export const WorkoutSelector = ({ 
  ...
  const handleWorkoutClick = async (workoutId: string) => {
    if (selectedWorkoutId === workoutId) {
      onWorkoutSelect(null); // Deselect if already selected
      resetWorkoutSession(); // Reset workout flow when deselecting
    } else {
      onWorkoutSelect(workoutId); // Select new workout
    }
  };
  ...
  const handleAdHocClick = async () => {
    if (selectedWorkoutId === 'ad-hoc') {
      ...
      onWorkoutSelect('ad-hoc'); // Select ad-hoc workout
    }
  };
