...
import { Info, PlusCircle, Heart, Home, LayoutTemplate, Edit, Trash2 } from 'lucide-react';
...
export const ExerciseListInfoDialog = ({ open, onOpenChange, type }: ExerciseListInfoDialogProps) => {
  ...
  const description = type === 'my-exercises' ?
    "This section displays exercises you've created or adopted. Here's how to use it:" :
  ...
  const content = type === 'my-exercises' ? (
    <div className="space-y-4">
      ...
      <div>
        ...
        <ul className="list-disc list-inside text-sm text-muted-foreground space-y-1">
          ...
          <li className="flex items-center"><Heart className="h-4 w-4 mr-2 flex-shrink-0" /> Mark/unmark as a favorite.</li>
    ...
    <div className="space-y-4">
      ...
      <div>
        <h4 className="font-semibold text-sm mb-1">Utilize Global Exercises:</h4>
        <ul className="list-disc list-inside text-sm text-muted-foreground space-y-1">
          <li className="flex items-center"><Info className="h-4 w-4 mr-2 flex-shrink-0" /> View detailed information about an exercise.</li>
          <li className="flex items-center"><Heart className="h-4 w-4 mr-2 flex-shrink-0" /> Mark/unmark as a favorite.</li>
          <li className="flex items-center"><PlusCircle className="h-4 w-4 mr-2 flex-shrink-0" /> Add to one of your workout templates (T-Paths).</li>
      ...
      <div>
        <h4 className="font-semibold text-sm mb-1">Customization:</h4>
        <p className="text-sm text-muted-foreground">
          You cannot directly edit or delete global exercises. If you want to customize a global exercise, you can add it to "My Exercises" first, which creates a copy you can then modify.
        </p>
      </div>
    </div>
  );
