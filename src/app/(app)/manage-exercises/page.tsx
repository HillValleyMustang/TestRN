...
export default function ManageExercisesPage() {
  ...
  const handleDeleteExercise = async (exerciseId: string) => {
    ...
    const { error } = await supabase.from('exercise_definitions').delete().eq('id', exerciseId);
    if (error) {
                    ...
                    {exercises.map((ex) => (
                      <li key={ex.id} className="flex items-center justify-between p-2 border rounded-md">
                        ...
                        <div className="flex space-x-1">
                          ...
                          <Button variant="ghost" size="icon" onClick={() => handleDeleteExercise(ex.id)}><Trash2 className="h-4 w-4 text-destructive" /></Button>
                        </div>
                      </li>
                    ))}
                  </ul>
