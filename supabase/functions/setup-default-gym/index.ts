...
serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: gym, error: gymError } = await supabaseServiceRoleClient
      .from('gyms')
      .select('id, user_id')
      .eq('id', gymId)
      .eq('user_id', user.id)
    ...
    if (!programmeType) throw new Error('User has no core programme type set.');

    // 3. Create a new main T-Path for the gym
    const tPathTemplateName = programmeType === 'ulul' ? '4-Day Upper/Lower' : '3-Day Push/Pull/Legs';
    ...
    // 4. Invoke generate-t-path for the new T-Path
    ...
    if (invokeError) {
      console.error("Failed to trigger default workout generation for new gym:", invokeError);
    }
