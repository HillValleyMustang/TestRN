...
serve(async (req: Request) => {
  ...
  try {
    ...
    for (const gym of userGyms) {
      ...
      // --- NEW: Determine original equipmentMethod for this gym's T-Path ---
      let originalEquipmentMethod: 'photo' | 'skip' = 'skip'; // Default to 'skip' if no existing T-Path
      const { data: existingMainTPath, error: fetchExistingTPathError } = await supabaseServiceRoleClient
        .from('t_paths')
        .select('settings')
        .eq('user_id', user.id)
        .eq('gym_id', gymId)
      ...
      if (fetchExistingTPathError && fetchExistingTPathError.code !== 'PGRST116') { // PGRST116 = no rows found, which is acceptable
        console.error(`Error fetching existing main T-Path settings for gym ${gymId}:`, fetchExistingTPathError.message);
        // Continue with default 'skip' if error, or throw if critical
      } else if (existingMainTPath?.settings && typeof existingMainTPath.settings === 'object' && 'equipmentMethod' in existingMainTPath.settings) {
        originalEquipmentMethod = (existingMainTPath.settings as { equipmentMethod: 'photo' | 'skip' }).equipmentMethod;
      }
      const useStaticDefaults = originalEquipmentMethod === 'skip';
      console.log(`[regenerate-all-user-plans] Gym ${gymId}: Original equipmentMethod: ${originalEquipmentMethod}, useStaticDefaults: ${useStaticDefaults}`);
      // --- END NEW ---
      ...
      const { data: newTPath, error: newTPathError } = await supabaseServiceRoleClient
        ...
        .insert({
          ...
          settings: {
            ...
            equipmentMethod: originalEquipmentMethod, // --- NEW: Use originalEquipmentMethod ---
          },
          is_bonus: false,
      ...
      // --- NEW: Pass useStaticDefaults to generateWorkoutPlanForTPath ---
      await generateWorkoutPlanForTPath(supabaseServiceRoleClient, user.id, newTPath.id, profile.preferred_session_length, gymId, useStaticDefaults);
