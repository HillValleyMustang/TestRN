...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (sourceExercises.length > 0) {
      ...
      await supabaseServiceRoleClient.from('gym_exercises').delete().eq('gym_id', targetGymId); // Clear old links first
      const { error: insertLinksError } = await supabaseServiceRoleClient.from('gym_exercises').insert(linksToCreate);
      if (insertLinksError) throw insertLinksError;
    }

    // 2. Fetch user profile to get necessary settings for new plan
    ...
    // 3. Create a new main T-Path for the target gym based on profile settings
    ...
    const { data: newTPath, error: newTPathError } = await supabaseServiceRoleClient
      ...
      .insert({
        user_id: user.id,
        gym_id: targetGymId,
        template_name: tPathTemplateName,
        settings: { tPathType: profile.programme_type, experience: 'intermediate', goalFocus: profile.primary_goal, preferredMuscles: profile.preferred_muscles, constraints: profile.health_notes, equipmentMethod: 'copy' },
        is_bonus: false,
    ...
    // 4. Generate the full workout plan for this new T-Path
    await generateWorkoutPlanForTPath(supabaseServiceRoleClient, user.id, newTPath.id, profile.preferred_session_length, targetGymId);

    // 5. Update profile to make the new gym and T-Path active
    await supabaseServiceRoleClient.from('profiles').update({ active_gym_id: targetGymId, active_t_path_id: newTPath.id }).eq('id', user.id);
