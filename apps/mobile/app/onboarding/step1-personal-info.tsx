...
export default function Step1PersonalInfo({ data, onDataChange, onNext }: Step1Props) {
  ...
  const validate = () => {
    ...
    if (!data.weight || data.weight < 30 || (data.weightUnit === 'kg' && data.weight > 300) || (data.weightUnit === 'lbs' && data.weight > 660)) {
      newErrors.weight = data.weightUnit === 'kg' ? 'Weight must be between 30-300 kg' : 'Weight must be between 66-660 lbs';
    }
  ...
  const isValid = data.fullName.trim() !== '' && 
    data.weight !== null &&
    (data.heightUnit === 'cm' ? (data.heightCm && data.heightCm >= 100 && data.heightCm <= 250) : 
     (data.heightFt && data.heightFt >= 3 && data.heightFt <= 8));

  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      <Text style={styles.title}>Let's Get to Know You</Text>
      <Text style={styles.subtitle}>Your personal details help us tailor your experience</Text>
      ...
      <View style={styles.section}>
        ...
        {data.heightUnit === 'cm' ? (
          <TextInput
            ...
            onChangeText={(text) => updateData('heightCm', text ? parseInt(text) : null)}
            placeholder="e.g., 175"
            placeholderTextColor="#666"
          ...
          <View style={styles.row}>
            <View style={styles.halfInput}>
              <TextInput
                ...
                onChangeText={(text) => updateData('heightFt', text ? parseInt(text) : null)}
                placeholder="Feet"
                placeholderTextColor="#666"
                keyboardType="numeric"
              />
            ...
            <View style={styles.halfInput}>
              <TextInput
                ...
                onChangeText={(text) => updateData('heightIn', text ? parseInt(text) : null)}
                placeholder="Inches"
                placeholderTextColor="#666"
                keyboardType="numeric"
              />
        ...
        <View style={styles.rowHeader}>
          ...
          <View style={styles.unitToggle}>
            ...
            <Switch
              ...
              onValueChange={(val) => updateData('weightUnit', val ? 'lbs' : 'kg')}
              trackColor={{ false: '#10B981', true: '#10B981' }}
              thumbColor="#fff"
            />
            <Text style={[styles.unitText, data.weightUnit === 'lbs' && styles.unitTextActive]}>lbs</Text>
          </View>
        </View>
        <TextInput
          style={[styles.input, errors.weight && styles.inputError]}
          value={data.weight?.toString() || ''}
          onChangeText={(text) => updateData('weight', text ? parseInt(text) : null)}
      ...
      <View style={styles.section}>
        ...
        <TextInput
          ...
          onChangeText={(text) => updateData('bodyFatPct', text ? parseInt(text) : null)}
          placeholder="e.g., 15"
          placeholderTextColor="#666"
          keyboardType="numeric"
        />
