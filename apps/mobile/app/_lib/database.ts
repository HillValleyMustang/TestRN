...
class Database {
  ...
  async init(): Promise<void> {
    ...
    this.initPromise = (async () => {
      debugLog('Starting database initialization with MIGRATION BLOCKING');

      try {
        this.db = await SQLite.openDatabaseAsync(DB_NAME);
        debugLog('Database connection opened');
      } catch (error: any) {
      ...
      debugLog('Starting MIGRATION BLOCKING SYSTEM...');
      await this.runMigrations();
      this.migrationComplete = true;
      debugLog('MIGRATION COMPLETE - Creating standard tables...');
  ...
  private async runMigrations(): Promise<void> {
    ...
    this.migrationPromise = (async () => {
      ...
      debugLog('🔄 MIGRATION BLOCKING: Starting comprehensive migrations');
      debugLog(`📅 Migration start time: ${new Date().toISOString()}`);
      ...
      while (!migrationSuccessful && retryCount < maxRetries) {
        try {
          ...
          debugLog('🔧 PHASE 2: Migrating t_path_exercises table...');
          ...
          if (!tPathExercisesColumns.includes('template_id')) {
            ...
            debugLog('✅ Created fresh t_path_exercises table with template_id');
            
            // Restore data if any existed (using t_path_id -> template_id mapping)
            if (existingData.length > 0) {
              debugLog(`🔄 Restoring ${existingData.length} t_path_exercises records...`);
              ...
              for (const record of existingData) {
                try {
                  ...
                  if (__DEV__) {
                    console.warn('⚠️  Failed to restore t_path_exercises record:', restoreError.message);
                  }
                }
              }
              
              debugLog('✅ Completed t_path_exercises data restoration');
            }
          } else {
            debugLog('✅ t_path_exercises table already has template_id column - no migration needed');
          }
          ...
          debugLog('✅ t_path_exercises final columns:', tPathExercisesFinalColumns);
          ...
          debugLog('🔧 PHASE 2.5: Migrating t_paths table...');
          try {
            ...
            debugLog('📋 Current t_paths columns:', tPathsColumns);
            ...
            if (tPathsMissingColumns.length > 0) {
              ...
              debugLog('✅ No missing t_paths columns - table already complete');
            }
            ...
            debugLog('✅ Final t_paths columns:', afterTPathsColumns);
            
          } catch (tPathsError: any) {
            console.error('❌ t_paths table migration failed:', tPathsError.message);
          ...
          debugLog('🔧 PHASE 2.6: Migrating workout_sessions table...');
          try {
            ...
            debugLog('📋 Current workout_sessions columns:', workoutSessionsColumns);

            // Add sync_status column if missing
            if (!workoutSessionsColumns.includes('sync_status')) {
              debugLog('➕ Adding sync_status column to workout_sessions table');
              await db.execAsync('ALTER TABLE workout_sessions ADD COLUMN sync_status TEXT DEFAULT "local_only"');
              debugLog('✅ Successfully added sync_status column to workout_sessions');
            } else {
              debugLog('✅ sync_status column already exists in workout_sessions table');
            }

            // Add gym_id column if missing (for ad-hoc workouts to store active gym)
            if (!workoutSessionsColumns.includes('gym_id')) {
              debugLog('➕ Adding gym_id column to workout_sessions table');
              await db.execAsync('ALTER TABLE workout_sessions ADD COLUMN gym_id TEXT');
              debugLog('✅ Successfully added gym_id column to workout_sessions');
            } else {
              debugLog('✅ gym_id column already exists in workout_sessions table');
            }
            ...
            debugLog('✅ Final workout_sessions columns:', afterWorkoutSessionsColumns);

          } catch (workoutSessionsError: any) {
            console.error('❌ workout_sessions table migration failed:', workoutSessionsError.message);
            // Continue without failing - workout_sessions errors are less critical
          }

          // PHASE 3: Migration profiles table - add missing columns
          debugLog('🔧 PHASE 3: Migrating profiles table...');
          try {
            // First, try ALTER TABLE approach
            debugLog('🔄 Attempting ALTER TABLE approach for profiles...');
            
            const profilesTableInfo = await db.getAllAsync<any>("PRAGMA table_info(profiles);");
            const profileColumns = profilesTableInfo.map((col: any) => col.name);
            debugLog('📋 Profiles columns before ALTER:', profileColumns);
            ...
            if (missingColumns.length > 0) {
              debugLog('➕ Adding ALL missing columns via ALTER TABLE:', missingColumns);
              
              for (const column of missingColumns) {
                try {
                  debugLog(`🔧 Adding column: ${column}`);
                  
                  if (column === 'user_id') {
                    await db.execAsync('ALTER TABLE profiles ADD COLUMN user_id TEXT');
                    debugLog('✅ Successfully added user_id column');
                  } else if (column === 'target_date') {
                    await db.execAsync('ALTER TABLE profiles ADD COLUMN target_date DATE');
                    debugLog('✅ Successfully added target_date column');
                    ...
                    debugLog('✅ Successfully added preferred_weight_unit column');
                    ...
                    debugLog('✅ Successfully added rolling_workout_status column');
                  } else if (column === 'active_location_tag') {
                    await db.execAsync('ALTER TABLE profiles ADD COLUMN active_location_tag TEXT');
                    debugLog('✅ Successfully added active_location_tag column');
                  }
                } catch (alterError: any) {
                  if (__DEV__) {
                    console.error(`❌ Failed to add ${column} column:`, alterError.message);
              ...
              debugLog('✅ No missing columns - profiles table already complete');
            }
            ...
            debugLog('📋 Profiles columns after ALTER:', afterAlterColumns);
            ...
            if (criticalMissing.length > 0) {
              debugLog('⚠️  CRITICAL: Still missing columns after ALTER TABLE:', criticalMissing);
              debugLog('🔄 Falling back to table recreation method...');
              
              await this.recreateProfilesTable(db);
            } else {
              debugLog('✅ ALTER TABLE migration successful - all critical columns present');
            }
            
          } catch (profilesError: any) {
            console.error('❌ Profiles ALTER TABLE migration failed:', profilesError.message);
            debugLog('🔄 Falling back to table recreation method...');
          ...
          debugLog('🔍 PHASE 4: Final migration verification...');
          ...
          debugLog('✅ Final t_path_exercises columns:', finalTPathExercisesColumns);
          ...
          debugLog(`✅ t_path_exercises table operational (${tPathExercisesTest[0]?.count || 0} records)`);
          ...
          try {
            ...
            debugLog(`✅ Profiles table operational (${profilesTest[0]?.count || 0} records)`);
            ...
            debugLog('✅ Column test result:', columnTest);
            
          } catch (profilesTestError: any) {
            console.error('❌ Profiles table test failed:', profilesTestError.message);
          }
          
          debugLog('🎉 MIGRATION SUCCESS: All migrations completed successfully');
          ...
          if (retryCount >= maxRetries) {
            console.error('🚨 CRITICAL: ALL MIGRATION ATTEMPTS FAILED');
          ...
          debugLog('⏳ Waiting 500ms before retry...');
          await new Promise(resolve => setTimeout(resolve, 500));
        }
      }
      
      debugLog('🏁 MIGRATION BLOCKING COMPLETED - All database schema mismatches resolved');
      debugLog(`📅 Migration completion time: ${new Date().toISOString()}`);
    })();
  ...
  isInitialized(): boolean {
    return this.db !== null && this.migrationComplete;
  }
  ...
  async addWorkoutSession(session: WorkoutSession): Promise<void> {
    ...
    await db.runAsync(
      `INSERT OR REPLACE INTO workout_sessions
       (id, user_id, session_date, template_name, completed_at, rating, duration_string, t_path_id, created_at, sync_status, gym_id)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        session.id,
        session.user_id,
  ...
  async saveUserPreferences(
    ...
    try {
      ...
      console.error('[Database] Error saving user preferences:', error);
      throw error;
    }
  }
    ...
    try {
      ...
      const userDataTables = [
        ...
        },
        {
          name: 'gyms',
          operation: async () => {
            // Always preserve at least one gym - users should never have 0 gyms
            // Get the oldest gym to keep as a fallback
            const gymsToKeep = await db.getAllAsync<{ id: string }>(
              'SELECT id FROM gyms WHERE user_id = ? ORDER BY created_at ASC LIMIT 1',
              [userId]
            );
            
            if (gymsToKeep && gymsToKeep.length > 0) {
              // Delete all gyms except the oldest one
              await db.runAsync('DELETE FROM gyms WHERE user_id = ? AND id != ?', [
                userId,
                gymsToKeep[0].id,
              ]);
              result.cleanedTables.push('gyms (preserved 1)');
              console.log('[Database] Preserved oldest gym during cleanup:', gymsToKeep[0].id);
            } else {
              // No gyms exist, nothing to delete
              console.log('[Database] No gyms to clean up for user:', userId);
            }
          }
        },
        {
